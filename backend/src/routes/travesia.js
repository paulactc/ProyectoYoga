...
router.post('/plan', verifyToken, async (req, res) => {
  try {
    ...
    const selectCols = "plan_type, DATE_FORMAT(start_date, '%Y-%m-%d') AS start_date, plan_days";
    let planResult = await executeQuery(
      `SELECT ${selectCols} FROM travesia_plans WHERE usuario_id = ?`,
      [req.user.id]
    );
    // Fallback: si plan_days no existe en la BD, seleccionar sin ella
    if (!planResult.success && planResult.error?.includes('plan_days')) {
      planResult = await executeQuery(
    ...
    if (!planResult.success || !planResult.data?.[0]) {
      return res.status(500).json({ success: false, error: planResult.error || 'No se pudo leer el plan guardado' });
    }
    res.json({ success: true, data: planResult.data[0] });
  } catch (err) {
    console.error('Error en POST /plan:', err);
